...
\end{equation}
gdzie $S_1 \dot{\setminus} S_2$ oznacza...\JL{Uzupełnic}.
...
\JL{cos chyba nie tak z numeracja, 4.31?, odniesnienia przez \ref{}}
...
w rozdziale...\JL{uzupełnic}. Zwróćmy, również uwagę, iż dzięki zastosowaniu techniki \textit{stochastycznego uśredniania}
...
Umożliwi nam to wersja tego algorytmu zaprezentowana na końcu podrozdziału....\JL{Uzupelnić}.
...
\end{equation}
Powyższy wzór wynika z rozumowania analogicznego do z.... \JL{podać numer wzoru, gdzie jest to pokazame}.
...
\JL{(zobacz: cytowanie pracy ''On Delta-Method of Moments and Probabilistic Sums'' J.Cichonia)},
 pozwalającej na oszacowanie wariancji funkcji zmiennej losowej, przy odpowiednich założeniach:
\begin{equation}
   Var(f(X)) \approx (f'(E[X]))^{2}Var(X).
\end{equation}
Oznaczmy przez $b_i = \frac{1}{{\hat{\beta}}_i} = \hat{J}(A_i, A_1 \cup A_2)$, oraz niech $f(x) = \frac{1}{x}$. Wówczas ${{\hat{\beta}}_i} = f(b_i)$. Podstawiając do wzoru $(4.43)$\JL{numeracja wzorow sie nie zgadza}
 otrzymujemy:
\begin{flalign}
...
\JL{zwnou numer wzoru sie nie zgadza, prosze w calej pracy to sprawdzic i zrobic odniesienia przez ref}
...
Aby policzyć wariancję dla estymatorów składowych przekroju ${{\hat{N}}_i}^{HLL}(S_1 \cap S_2) = \hat{{\alpha}_i}{{\hat{N}}_i}^{HLL}(S_i)$  wykorzystamy wzór $(4.11)$.
\JL{zmienic na ref odniesienie do wzoru}
Zauważmy, że estymator w \texttt{HyperLogLog} zbiega do rozkładu normalnego, 
patrz rozdział...\JL{podac rozzdial z HLL}.
...
\JL{odniesienie do wzoru sprawdzic}
