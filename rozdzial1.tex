...
Szkicem danych algorytmu \texttt{HyperLogLog} jest kolekcja rejestrów $M[i]$ przechowujących największą napotkaną pozycję pierwszej jedynki w haszach podstrumienia $\mathfrak{M}_i$, gdzie $i = 1 ... m$, $m = 2^p$ wyznacza liczbę rejestrów. Taki szkic danych zajmuje $O(\epsilon^{-2}\log{\log{n}} + \log{n})$ pamięci, gdzie $\epsilon = \frac{1.04}{\sqrt{m}}$ to błąd standardowy algorytmu \cite{hll}.
